...
ll build(int ind, int low, int high){
...
ll update(int ind, int low, int high, int target, int value){
    if(target < low || high < target) return seg[ind];
    if(low==high) return seg[ind] = value;
    int mid = (low+high)/2;
    return seg[ind] = update(2*ind, low, mid, target, value) + update(2*ind+1, mid+1, high, target, value);
}
